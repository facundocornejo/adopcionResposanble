...
interface CasoExitoFormData {
  titulo: string
  historia: string
  fecha_adopcion: string
}

interface AnimalWithStatus extends Animal {
  newStatus?: string
}

export interface CasoExitoModalProps {
  isOpen: boolean
  onClose: () => void
  animal: AnimalWithStatus | null
  onSuccess?: (casoCreated: boolean) => void
}

/**
 * Modal para crear un caso de éxito cuando un animal es adoptado
 */
function CasoExitoModal({ isOpen, onClose, animal, onSuccess }: CasoExitoModalProps) {
  ...
  const onSubmit = async (data: CasoExitoFormData) => {
    if (!animal) return

    setIsSubmitting(true)

    try {
      const response = await api.post<{ success: boolean }>('/api/casos-exito', {
        animal_id: animal.id,
        titulo: data.titulo,
      ...
      toast.error(axiosError.response?.data?.error?.message || 'Error al crear el caso de éxito')
    } finally {
