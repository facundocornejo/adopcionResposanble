...
import { Button, Input, Textarea, Card, Alert } from '@/presentation/components/ui'
import api from '@/infrastructure/api/axios-client'
import type { CreateOrganizacionInput, CreateOrganizacionResponse } from '@/domain/entities/super-admin'
...
function SuperAdminNewOrg() {
  ...
  const [createdData, setCreatedData] = useState<CreateOrganizacionResponse | null>(null)

  const {
    register,
    handleSubmit,
    formState: { errors }
  } = useForm<CreateOrganizacionInput>()

  const onSubmit = async (data: CreateOrganizacionInput) => {
    setIsSubmitting(true)
    try {
      const response = await api.post<ApiResponse<CreateOrganizacionResponse>>('/api/super-admin/organizations', data)
