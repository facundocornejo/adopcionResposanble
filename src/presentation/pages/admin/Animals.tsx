...
import { ESPECIES, ESTADOS_ANIMAL } from '@/shared/constants'
import { Button, Badge, Spinner, Alert, Modal, Card } from '@/presentation/components/ui'
import { CasoExitoModal } from '@/presentation/components/admin'
import type { Animal, AnimalEstado, AnimalFilters } from '@/domain/entities/animal'
...
interface CasoExitoModalState {
  isOpen: boolean
  animal: (Animal & { newStatus?: string }) | null
}
...
function Animals() {
  ...
  const [casoExitoModal, setCasoExitoModal] = useState<CasoExitoModalState>({
    isOpen: false,
    animal: null,
  })
  ...
  const fetchAnimals = useCallback(async () => {
    ...
    try {
      const data = await animalApi.getAll(filters)
      setAnimals(data.animales)
    } catch (err) {
      const error = err as Error
      setError(error.message || 'Error al cargar animales')
    } finally {
      setIsLoading(false)
    }
  }, [filters])
  ...
  const handleStatusChange = async (animal: Animal, newStatus: AnimalEstado) => {
    ...
    try {
      await animalApi.updateStatus(animal.id, newStatus)
  ...
  const handleCasoExitoSuccess = async () => {
    const animal = casoExitoModal.animal
    if (!animal) return

    try {
      // Siempre actualizar el estado a Adoptado
      await animalApi.updateStatus(animal.id, 'Adoptado')
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) => handleFilterChange('busqueda', e.target.value)}
          ...
          <select
            ...
            onChange={(e: ChangeEvent<HTMLSelectElement>) => handleFilterChange('especie', e.target.value)}
          ...
          <select
            ...
            onChange={(e: ChangeEvent<HTMLSelectElement>) => handleFilterChange('estado', e.target.value)}
            className="px-4 py-2.5 border border-brown-200 rounded-xl focus:outline-none focus:border-terracotta-500 transition-colors bg-white"
          >
                ...
                {animals.map((animal) => (
                  <tr key={animal.id} className="hover:bg-warm-50 transition-colors">
                    ...
                    <td className="p-4">
                      <select
                        value={animal.estado}
                        onChange={(e: ChangeEvent<HTMLSelectElement>) => handleStatusChange(animal, e.target.value as AnimalEstado)}
